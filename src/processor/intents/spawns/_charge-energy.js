...
function oldEnergyHandling(spawn, cost, {roomObjects, bulk}){
    ...
    var availableEnergy = _.sumBy(_.values(_.map(extensions, v => +v)), 'store.energy') + _.sumBy(_.values(_.map(spawns, v => +v)), 'store.energy');
...
function newEnergyHandling(spawn, cost, energyStructures, {roomObjects, bulk}){
    ...
    let availableEnergy = _.sumBy(_.values(_.map(energyStructures, v => +v)), id => roomObjects[id].store.energy);
    if(availableEnergy < cost) {
        return false;
    }
